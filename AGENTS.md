...
- Superpowers is disabled and must not be invoked, rediscovered, updated, or reinstalled unless the user explicitly reverses this decision. Do not classify work as requiring a Superpowers escalation layer.
- Complex, ambiguous, risky, or long-running work uses native planning and diagnosis, the narrowest useful Matt Skill when one matches, and proportional independent review, QA, or security checks. Choose one primary workflow for each phase and avoid overlapping workflows for ceremony.
...
- Provider roles are replaceable: native execution, routine engineering, and proportional review/QA/security are semantic roles rather than permanent model-specific trees.

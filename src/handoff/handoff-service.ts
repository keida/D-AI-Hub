...
import { createHandoffEnvelope, parseHandoffEnvelope, validateHandoffCreateInput, type HandoffEnvelope } from "./envelope.js";
...
const persistedRecordSchema = z.object({ envelope: z.object({}).passthrough(), owner: environmentSchema.nullable(), state: z.enum(["pending", "active", "completed", "rejected"]), reason: z.string().nullable() }).strict();
...
function parsePersistenceRecords(value: object): readonly HandoffPersistenceRecord[] {
  ...
  return result.data.records.map((record) => {
    const envelope = parseHandoffEnvelope(record.envelope as never as HandoffEnvelope);
...
export class FileHandoffPersistence implements HandoffPersistence {
  ...
  public async load(): Promise<readonly HandoffPersistenceRecord[]> {
    ...
    try {
      const document = persistenceDocumentSchema.safeParse(JSON.parse(content) as object);
...
export class PersistentHandoffService implements HandoffService {
  ...
  public async create(input: { readonly state: TaskState; readonly targetEnvironment: Environment }): Promise<HandoffEnvelope> {
    const validated = validateHandoffCreateInput({ handoffId: "handoff-validation-1", state: input?.state, targetEnvironment: input?.targetEnvironment });
  ...
  public async ready(): Promise<void> { await this.initialize(); }

  private async initialize(): Promise<void> { if (this.initialized) return; const records = await this.persistence.load(); for (const record of records) { if (this.records.has(record.envelope.handoffId)) throw new InvalidHandoffError(`Duplicate persisted handoff id: ${record.envelope.handoffId}`); this.records.set(record.envelope.handoffId, cloneRecord(record)); } this.initialized = true; }

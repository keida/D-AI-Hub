...
export class WorkEnvironmentAdapter {
  ...
  public async receive(envelope: HandoffEnvelope): Promise<void> {
    if (this.activationConnector === null) throw new InvalidTaskStateError("Work activation connector is not configured");
  ...
  public async execute(request: EnvironmentExecutionRequest): Promise<EnvironmentExecutionResult> {
    if (this.executor === null) {
      throw new InvalidTaskStateError("Work execution adapter is not configured");
    }

...
This amendment adds a single-operator, local-first authority mode to the 2026-09-24 DAI-ARCH-002 result-contract decision. It supersedes that decision's verified Project Boss-origin proof requirement only for D-AI reconciliation authorization in `HUMAN-CONFIRMED` mode. Project-side execution and acceptance authority remain the project owner's responsibility; this mode does not identify the human as that project's Boss. The existing project-owned result v1 envelope and schema remain unchanged.

**Authority states**

- `UNVERIFIED` is the default. A result, JSON confirmation-shaped document, repository permission, Git identity, or chat statement alone remains unverified and cannot authorize reconciliation.
- `HUMAN-CONFIRMED` means the local human operator explicitly confirms the intent to reconcile one exact immutable project result after being shown its complete binding. This records human intent; it is not cryptographic identity, a verified Project Boss context, proof of Git authorization, project acceptance, D-AI verification, or permission to close.
- `HOST-ATTESTED` is reserved for a future automated host-attestation mode and is not implemented or accepted by this contract amendment.

The confirmation event shape is [project-owned-result-confirmation-v1.schema.json](contracts/project-owned-result-confirmation-v1.schema.json). A conforming JSON object is only event data: it is insufficient unless a future D-AI interaction obtains the operator's explicit confirmation and creates the event. Pasting a chat statement or supplying a standalone JSON file does not satisfy that requirement. No confirmation event or capture mechanism is created by this decision.

**Exact binding and replay**

The operator confirms the intent `RECONCILE_PROJECT_OWNED_RESULT` for one binding containing canonical project identity, project work ID and execution-owner ID, D-AI task ID, result ID, result contract and schema version, canonical repository identity and repository-relative artifact path, publication commit, artifact Git blob OID and SHA-256, and the exact immutable evidence-reference set. Evidence reference IDs must be unique; each reference's immutable locator and digest must match the result, with the set compared independent of array order. The event also carries a unique event ID, confirmation timestamp, and local interaction context. The consumer must compare every bound value with the immutable published result and current task before treating the event as eligible. Any mismatch, changed binding, malformed or stale event, or reused event ID with a different payload fails closed. A future runtime must durably record the event ID and complete canonical payload with its replay-prevention outcome before acknowledging eligibility. Replaying the same event ID with the same canonical payload is deterministic and idempotent; it does not repeat an action or broaden its scope. A changed binding requires a separate explicit human confirmation.

Session and interaction references are minimal audit context only. They do not establish a verified person, Project Boss role, session identity, or cryptographic origin. `HOST-ATTESTED` would require a separately designed and reviewed trust mechanism.

Any future implementation must keep mode-specific authority checks behind a thin, replaceable `AuthorityVerifier` boundary, with `HumanConfirmationVerifier` for `HUMAN-CONFIRMED` and a future `HostAttestationVerifier` for `HOST-ATTESTED`. Authority-mode logic must not spread into canonical identity, memory, recovery, durable-store core, or ordinary lifecycle transitions. This is an architectural constraint only; neither the interface nor its implementations are added here.

**Lifecycle boundary and example**

`HUMAN-CONFIRMED` makes the exact result eligible for a separate future reconciliation operation; it does not itself perform reconciliation. It creates no `VerificationEvidence`, does not satisfy existing verification gates, does not authorize a new lifecycle stage, and does not change the ordinary prohibition on `route → verify`. Close remains a distinct decision subject to existing close gates. No runtime reconciliation, close API, or Skill Pulse result consumption is implemented or authorized by this amendment.

For schema illustration only, the supplied read-only metadata identifies the published [Skill Pulse SP-OPS-006 result at commit `c98c9a8d53263d1594fcfdf533e2c5851b0e2167`](https://github.com/keida/skill-pulse/blob/c98c9a8d53263d1594fcfdf533e2c5851b0e2167/docs/project-results/SP-OPS-006.project-owned-result-v1.json), Git blob `0cfcff0ddff5c65208febe3c6d66582db7fd4f93`, artifact SHA-256 `906000dfd034b77105a7bfe5058d65b52ef37caa29ca4d5e54f655679d6abec0`. The supplied metadata reports result ID `skill-pulse.sp-ops-006.20260922.retrospective.v1`, project `github.com/keida/skill-pulse`, task `task-7b959c2155241137b16a5220`, reported execution `PASS`, reported acceptance `ACCEPTED`, and eight evidence references. These are references to published artifact metadata, not a new acceptance claim or D-AI verification; this amendment did not consume or independently revalidate the artifact.
...
Project-side execution and acceptance, approval of the project-owned canonical result location, and authorization of publication remain the declared project's responsibility under both authority modes. The project's declared Project Boss approves the result and any delegated Git writer through that project's controls; delegation and publication must remain auditable. A string `ownerId`, repository write access, Git commit/hash, or typed JSON alone does not prove project acceptance or publication authority.

For `HOST-ATTESTED` mode only, approval and postpublication attestation must come from a verified bound Project Boss execution context for that project and task. Its authority proof must bind the exact `resultId`, project/task IDs, publication commit, registered path, blob OID, and artifact SHA-256; it must be scoped, fresh, and replay-resistant. D-AI must durably record proof consumption or equivalent replay-prevention state before acknowledging it. This proof mode is future and unimplemented; missing or invalid proof blocks `HOST-ATTESTED`.

For `HUMAN-CONFIRMED` mode, D-AI does not claim it independently verified Project Boss identity or Git authorization. A separately captured explicit operator-intent event authorizes only consideration of the exact already-published result for reconciliation. It does not replace the project's acceptance, canonical-location approval, or publication responsibilities and cannot authorize publication itself.

**D-AI consumer verification**

In either mode, the D-AI consumer verifies the existing durable task and current project identity, the project's approved canonical repository and registered result path, the exact published commit, blob OID, artifact bytes and SHA-256, the result schema, typed project acceptance fields, and the result's project/task/work binding. It independently verifies the immutable evidence references, their digests, profile-specific relationships, and the evidence supporting the claimed execution and acceptance. `HOST-ATTESTED` additionally requires the verified bound Boss-origin proof described above. `HUMAN-CONFIRMED` instead requires a separately captured operator-intent event bound to this exact project, task, result, artifact, and evidence set. Under HUMAN-CONFIRMED, D-AI verifies that the event was explicitly captured and durably recorded; it does not claim independent verification of Boss identity or Git authorization. Missing, malformed, stale, conflicting, cross-project, unregistered, or unverifiable input fails closed without writing a successful handoff. Replaying the same event ID with the same complete canonical binding is idempotent; changed event payload or a conflicting result cannot replace an accepted result silently.
...
Already-completed work is not backfilled from chat or from D-AI's interpretation of engineering evidence. The Project Boss must re-evaluate the preserved immutable evidence, issue a typed retrospective result through the project's acceptance and publication controls, record the actual new acceptance/publication time separately from the original run time, and publish it at its approved project-owned location. If required project acceptance evidence is unavailable, the result remains absent and D-AI remains blocked. Skill Pulse task `task-7b959c2155241137b16a5220` stays at `route / unclosed`; do not rerun SP-OPS-006 or alter its production pipeline. The present runtime has no HUMAN-CONFIRMED event capture or guarded reconciliation implementation. The later HUMAN-CONFIRMED amendment means the missing verified Project Boss caller identity blocks only the future HOST-ATTESTED mode; it does not provide runtime implementation authorization or consume an artifact. Any later Skill Pulse integration still needs a separately approved exact project-owned path and a separate implementation/consumption decision.

The follow-up is split: **DAI-ARCH-002A — Trusted Project Boss Invocation** defines the smallest verifiable Boss-origin runtime trust primitive for optional `HOST-ATTESTED` use; it is not a prerequisite for the HUMAN-CONFIRMED mode. **DAI-ARCH-002B — Skill Pulse Project-Owned Result Integration** covers a separately approved exact path and later consumption of the published typed result with immutable evidence under the selected authority mode. Neither follow-up is implemented or authorized for activation by publishing this contract.
